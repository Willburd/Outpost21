...
/datum/ai_holder
	...
	var/maximum_path_distance = 0		// distance AI will path too from the start location ( by default world.view*6 ), set in /datum/ai_holder/New()
...
/datum/ai_holder/proc/calculate_path(atom/A, get_to = 1)
	...
	get_path(get_turf(A), get_to, maximum_path_distance)

	ai_log("calculate_path() : Exiting.", AI_LOG_DEBUG)

//A* now, try to a path to a target
/datum/ai_holder/proc/get_path(var/turf/target,var/get_to = 1, var/max_distance)
