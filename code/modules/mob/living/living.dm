...
/mob/living/proc/slam_grabbed_mob_against_thing(var/obj/item/weapon/grab/G)
	...
	if(throw_mob && a_intent == I_HURT && G.state >= GRAB_NECK)
		...
		// randy, and drug check
		var/lacertusol = 0
		for(var/datum/reagent/phororeagent/R in reagents.reagent_list)
			if(R.id == "lacertusol")
				lacertusol = 1
				break
