...
/obj/machinery/transhuman/autoresleever/proc/autoresleeve(var/obj/item/weapon/card/id/D)
	...
	if(chosen_species.flags & NO_SCAN || recordM.hiderecord) // Sanity. Prevents species like Xenochimera, Proteans, etc from rejoining the round via resleeve, as they should have their own methods of doing so already, as agreed to when you whitelist as them.
	...
	if(tgui_alert(ghost, "Autoresleever has received the ID of '[D.registered_name]' for processing. Would you like to be resleeved? Remember to have that character loaded, or it will fail! This is your only chance to change slots!", "Resleeve", list("Yes","No")) == "No")
		if(isliving(ghost))
